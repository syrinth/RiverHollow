    ...
    public static class MapManager
    {
        ...
        public static void PopulateMaps(bool loaded)
        {
            ...
            if (!loaded)
            {
                for (int i = 0; i < 99; i++)
                {
                    _tileMaps[MapManager.HomeMap].PlaceWorldObject(DataManager.GetWorldObject(0, new Vector2(rand.Next(1, mapWidth - 1) * TileSize, rand.Next(1, mapHeight - 1) * TileSize)), true);
                }
                for (int i = 0; i < 99; i++)
                {
                    _tileMaps[MapManager.HomeMap].PlaceWorldObject(DataManager.GetWorldObject(2, new Vector2(rand.Next(1, mapWidth - 1) * TileSize, rand.Next(1, mapHeight - 1) * TileSize)), true);
                }
                for (int i = 0; i < 10; i++)
                {
