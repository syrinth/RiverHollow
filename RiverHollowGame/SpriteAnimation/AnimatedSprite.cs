...
namespace RiverHollow.SpriteAnimations
{
    public class AnimatedSprite
    {
        ...
        public void AddAnimation(string animationName, int startX, int startY, RHSize size, int Frames = 1, float FrameLength = 1f, bool pingPong = false, bool playsOnce = false) { AddAnimation(animationName, startX, startY, size.Width * TILE_SIZE, size.Height * TILE_SIZE, Frames, FrameLength, pingPong, playsOnce); }
        ...
        {
            if (!Drawing || CurrentAnimation != animate && _diFrameAnimations.ContainsKey(animate))
            {
                Drawing = true;
